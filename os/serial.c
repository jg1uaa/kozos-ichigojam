...
#define	UARTBase		0x40008000	// UART0に固定
#define	UARTClock		(CPU_CLOCK / 1)	// Hz
#define	UARTSpeed		38400		// bps
#define	UARTDivisor		(UARTClock / (UARTSpeed * 16))

#define	InReg(adr)		*((volatile unsigned long *)(adr))
#define	OutReg(adr, dat)	{*((volatile unsigned long *)(adr)) = (dat);}

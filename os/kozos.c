...
void dispatch(void);
const uint32 offsetof_kz_context = offsetof(kz_thread, context);
				 ...
				 int stacksize, int argc, char *argv[])
{
  ...
  /* スタックの初期化 */
  // スレッドの優先度がゼロの場合に，割込み禁止スレッドとする機能は実装不可．
  // (厳密には、一定のレベル以下の割込みを禁止することで、ディスパッチャと
  // して使用しているPendSVハンドラも呼び出せなくなってしまう)
  // スレッドのスタート・アップ(thread_init())に渡す引数は，R0に設定する．
  sp = (uint32 *)thp->stack;
  *(--sp) = (uint32)0x01000000; /* EPSR.T=1 */
  *(--sp) = (uint32)thread_init; /* R15(PC) */
  *(--sp) = (uint32)thread_end; /* R14(LR): スレッド終了時の戻り先 */
  *(--sp) = 0; /* R12 */
  *(--sp) = 0; /* R3 */
  *(--sp) = 0; /* R2 */
  *(--sp) = 0; /* R1 */
...
/* 割込み処理の入口関数 */
void thread_intr(softvec_type_t type)
{
  /* カレント・スレッドのコンテキストは、
   * ハードウェアによって既に一部が保存されている。
   * 残った部分はディスパッチャ（PendSVハンドラ）で保存する。 */
